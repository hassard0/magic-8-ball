...
Return the UUIDs (the "uuid" field) of documents that are NOT relevant. Return ONLY valid UUIDs, not index numbers.`,
        ...
        if (!response.ok) {
          console.error(`Relevance filter batch ${batchIdx + 1} failed:`, response.status);
          return [];
        }
        ...
        if (toolCall) {
          ...
          if (result.irrelevant_ids?.length > 0) {
            ...
            return result.irrelevant_ids.filter((id: string) => uuidRegex.test(id));
          }
        }
        return [];
      })
    ...
    for (const result of batchResults) {
      if (result.status === "fulfilled" && result.value.length > 0) {
        irrelevantIds.push(...result.value);
      }
    }
